...
class PaperAgentXHS():
    def __init__(self,platform:str='xiaohongshu',log_path='./logs/paperagent/paper_xhs_log.log'):
        ...
        self.paper_database = sql_dataset('papers')  # 论文数据库
        self.action_alias = {"仅评论": "评论"}  # 动作别称
    ...
    async def get_action_time(self,account_ids,url=None,history=None,current_time=None,is_predict=True):
        ...
        for account_id in account_ids:
            if account_ids.index(account_id) == 0:
                if is_predict:
                    planner = XiaohongshuPlanner(log_path=f"./logs/{self.platform}/{account_id}/planner_log.log")
    ...
    async def auto_cultivation(self,account_ids:list,domain:str=None,paper_path=None):
        ...
        # 每个账号要获取的 最新一条通知 的时间
        time_limit = datetime.now()- timedelta(days=3) # 第一次，获取3天前的通知
        ...
        while True:
            ...
            planner = XiaohongshuPlanner(log_path=f"./logs/{self.platform}/{account_ids[0]}/{self.platform}_log.log")
            ...
            while True:
                ...
                if accounts_dict:
                    ...
                    for account_id in accounts:
                        ...
                        bot = XiaohongshuBot(log_path=f"./logs/{self.platform}/{account_id}/{self.platform}_log.log")  
                        ...
                        if len(propaganda_results) !=  0 :
                            ...
                            select_nickname = f'''SELECT Nickname FROM accounts_info WHERE Account_id = {account_id}'''
                            account_nickname = self.database.get_dict_data_sql(select_nickname)[0]['Nickname']
                            # 将宣传的链接存入到数据库中
                        ...
                        # 加上小红书的获取通知
                        await bot.get_notifications(account_id=account_id,time_limit=accounts_notify_time[int(account_id)])
                        '''获取通知成功'''
                        await asyncio.sleep(random.uniform(5,10))
    ...
    async def auto_paper_single_account(self,account_id,paper_title:str=None,domain:str=None,paper_info:dict=None,bot=None):
        ...
        try:
                ...
                for keyword in paper_info['Gene_keywords'] + domain_keywords:
                    ...
                    while True:   
                        ...
                        if not post_info and not article_element:
                            ...
                            break # 继续查找下一个关键词
                        if datetime.strptime(post_info['post_time_ip'],"%Y-%m-%d %H:%M:%S") < today:
                            self.log.info(f'已经获取到前两天的贴文，不进行操作，更换关键词')
            ...
            if self.auto_post:
                # 再执行一次主动发帖宣传论文
                await asyncio.sleep(random.uniform(5,10))
                self.log.info(f'平台{self.platform}的账号{account_id}进行主动发帖')
                ...
                propaganda_results.append(propaganda_result)
                # '''发帖成功，则将论文数据库的状态更改，后续不再 宣传该篇论文'''
                # insert_sql = '''INSERT INTO papers_promotion (`Paper_id`,`Platform`,`Account_id`,`Account`,`Post_URL`,`Update_time`) VALUES (%s,%s,%s,%s,%s,%s);'''
                # self.paper_database.operation(insert_sql,(paper_info['Paper_id'],'xiaohongshu',account_id,account_nickname,post_url,datetime.now()))
                # self.log.info(f"平台{self.platform}账号{account_id}宣传论文{paper_info['Paper_id']}:{paper_info['Title']}成功，并成功将数据插入到数据库中")
    ...
    async def dispatch_action(self,bot,account_id,character:str,action:str,paper_info:dict,published:dict=None):
        ...
        try:
            ...
            if '评论' in action or '转发' in action:
                ...
                if action == '转发':
                    await bot.transmits(account_id=int(account_id),url=published["note_url"],content=content,file_paths=file_paths) 
                if action == '仅转发':
                    await bot.transmits(account_id=int(account_id),url=published["note_url"],content=content)   
                if action == '评论': 
                    await bot.comments(account_id=int(account_id),url=published["note_url"],content=content,file_paths=file_paths,post_data=published) 
                if action == '仅评论':
                ...
                await bot.likes(account_id = int(account_id),url = published["note_url"],post_data=published)
                post_url = None
            elif action == '关注':
                await bot.follows(account_id=int(account_id),url=published["note_url"])
                post_url = None
            elif action == '收藏':
                await bot.collects(account_id=int(account_id),url=published["note_url"],post_data=published)
                post_url = None
            elif action == '发帖':  # 主动发帖宣传论文
                text = f'\n论文标题是: {paper_info["Title"]} \n论文摘要是: {paper_info["Abstract"]}' 
                _,content = await general_generation_think([{"role": "system", "content": xhs_post_paper_prompt(character=character,language=self.language,max_length=800)},{"role": "user", "content": text}])    
                self.log.info(f'模型生成的发帖内容是：{content}')
                content = ''.join(c for c in content if ord(c) <= 0xFFFF) 
                content = self.cut_content(content,max_length=900)
                content += f'''\n\n arxiv: {paper_info["URL"]}  \n\n'''
                self.log.info(f'经过字数检查之后要发帖的内容是：{content}')
                post_url = await bot.posts(account_id=int(account_id),content=content,file_paths=paper_info['image_paths'],tags=['论文分享','论文推荐','推荐系统','LLM'])

            # 在该页面进行浏览
            bot.scroll(duration=random.uniform(8,15))
            await asyncio.sleep(random.uniform(10,20))
            # return post_url  # 发布的帖子/评论链接
            return {post_url:action} if post_url else None
        except Exception as e:
...
if __name__ == "__main__":
    ...
    # asyncio.run(agent.auto_cultivation(account_ids=[224,1],domain='Large Language Models for Recommendation',paper_path=r'D:\论文agent\2025-08-20'))   # 多个账号串行运营
    # 同一个领域的多个账号串行 224,
    agent = PaperAgentXHS(platform='xiaohongshu',log_path=f'./logs/paperagent/paperagent_llm4rec_log.log')
    asyncio.run(agent.auto_cultivation(account_ids=[10],domain='Large Language Models for Recommendation',paper_path=r'D:\论文agent\2025-08-20'))   # 多个账号串行运营
...
# nohup python paper_agent/paper_agent.py >> paper_agent/paper_agent_1029.log 2>&1 &
